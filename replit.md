...
- **Support Helpdesk**: Complete ticketing system with customer portal and admin console. Features include two-way communication (public replies + internal notes), task management for each ticket, SLA tracking with breach detection, team-based assignment (Support, Finance, Logistics, Quality, IT), event timeline for audit, multi-channel notifications (email, SMS, WhatsApp) for ticket events.
