...
- **Global Announcement System**: Admin-managed system announcements with 3 severity levels (INFO, WARNING, CRITICAL), 2 publish modes (IMMEDIATE, SCHEDULED), 4 status states (DRAFT, SCHEDULED, ACTIVE, EXPIRED), flexible audience targeting (ALL, ROLE-based, CUSTOMER-specific, USER-specific), multi-channel delivery (IN_APP, EMAIL, SMS, WHATSAPP), and per-user read/dismiss tracking. Admin management at `/admin/announcements` with list page featuring KPI cards (Total, Active, Scheduled, Drafts), filters, and inline actions (publish, unpublish, duplicate, delete). Create/edit form with audience builder, scheduling, and channel selection. Global `AnnouncementBar` component displays active announcements with auto-rotation (10s), severity-based styling, navigation arrows, count indicator, pause on hover, dismiss functionality, and external link support. Integrated into Layout, PortalLayout, and DriverLayout for system-wide visibility.
